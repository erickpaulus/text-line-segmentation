...
import glob
# from local_th import feng_threshold
# import color2gray
...
#         img_x = cv2.imread(fname_x)
#         img_y = cv2.imread(fname_y)
#         img_x = img_x[:,:,0]
#         img_y = img_y[:,:,0]

#        th = feng_threshold(img_x)		
#        img_x = apply_threshold(img_x, th)
#        th = feng_threshold(img_y)		
#        img_y = apply_threshold(img_y, th)

#        hsv_x = cv2.cvtColor(img_x, cv2.COLOR_BGR2HSV)
#        img_x=hsv_x[:,:,2]
#        hsv_y = cv2.cvtColor(img_y, cv2.COLOR_BGR2HSV)
#        img_y=hsv_y[:,:,2]  
...
def generate_chunks(array_x_files, x_sufix, y_sufix, window_size, step_size):
    ...
    for fname_x in array_x_files:
#         print("fname_x:",fname_x)

        fname_y = fname_x.replace(x_sufix, y_sufix)
#         print("fname_y:",fname_y)
#         print("xxX:",fname_y.split('.')[0]+".*")
        
        fname_y = glob.glob(fname_y.split('.')[0]+".*")[0]
        print("fname_y:",fname_y)
